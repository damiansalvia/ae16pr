...
SetUpParams::SetUpParams(Operator_Pool& pool) :
		_independent_runs(0), _nb_evolution_steps(0), _population_size(0), _population_additional_size(
				0), _select_parents(0), _select_offsprings(0), _parameter_select_parents(
				0), _parameter_select_offsprings(0), // Parameter of selection is fixed to 0
		_inter_operators(), _intra_operators(), _combine(1), _refresh_global_state(
				1), _synchronized(0), _check_asynchronous(1), _display_state(0), _pool(
				pool) {
}
	...
	while (is.getline(buffer, MAX_BUFFER, '\n')) {
		...
		case 0:
			...
			case 2:
				...
				break;
			case 3:
				setup.population_additional_size(op);
				break;
			case 4:
				setup.combine(op);
				break;
			...
			break;
		case 1:
			op = -1; // creates the chosen selection method
			parameter = 0;
...
ostream& operator<<(ostream& os, const SetUpParams& setup) {
	...
	os << "\t" << "Independent runs : " << setup.independent_runs() << endl
			...
			<< "\t" << "Size of Additional population: "
			<< setup.population_additional_size() << endl;
	if (setup.combine())
	...
	os << "\t" << "Display State: " << setup.display_state() << endl << endl
			...
			<< endl << "\t" << "Selection offsprings -> "
			<< setup.pool().selector(setup.select_offsprings()) << endl << "\t"
			<< "Parameter of selection: " << setup.parameter_select_offsprings()
			<< endl << endl << "\t" << "Intra_Operators: " << endl << "\t"
			<< "-----------" << endl << endl;

	for (int i = 0; i < setup.intra_operators_size(); i++)
		os << "\t"
				<< (setup.pool().intra_operator(setup.intra_operator_index(i)))
				<< endl;
	...
	for (int i = 0; i < setup.inter_operators_size(); i++)
		os << "\t" << "Operator: "
				<< setup.pool().inter_operator(setup.inter_operator_index(i))
				<< endl;
...
const unsigned int SetUpParams::intra_operator_index(
...
const unsigned int SetUpParams::inter_operator_index(
		const unsigned int index) const {
...
void SetUpParams::RefreshState(const StateCenter& _sc) const {
	...
	_sc.set_contents_state_variable("_parameter_select_parents",
			(char *) &_parameter_select_parents, 1,
			sizeof(_parameter_select_parents));
	_sc.set_contents_state_variable("_select_offsprings",
			(char *) &_select_offsprings, 1, sizeof(_select_offsprings));
	_sc.set_contents_state_variable("_parameter_select_offsprings",
			(char *) &_parameter_select_offsprings, 1,
			sizeof(_parameter_select_offsprings));
...
void SetUpParams::UpdateFromState(const StateCenter& _sc) const {
	...
	_sc.get_contents_state_variable("_parameter_select_parents",
			(char *) &_parameter_select_parents, nbytes, length);
	_sc.get_contents_state_variable("_select_offsprings",
			(char *) &_select_offsprings, nbytes, length);
	_sc.get_contents_state_variable("_parameter_select_offsprings",
			(char *) &_parameter_select_offsprings, nbytes, length);
...
Population::Population(const Problem& pbm, const SetUpParams& setup) :
		_parents(setup.population_size()), _fitness_values(
				setup.population_size()), _new_parents(setup.population_size()), _offsprings(
				setup.population_additional_size()), _setup(setup), _evaluations(
				0) {
...
void Population::select_parents() {
	...
	for (int i = 0; i < _offsprings.size(); i++) {
		ind = _setup.pool().selector(_setup.select_parents()).select_one(
				_parents, _offsprings, _fitness_values,
...
void Population::select_offsprings() {
	...
	for (int i = 0; i < ps; i++) {
		if (_setup.combine()) {
			aux[i] =
					_setup.pool().selector(_setup.select_offsprings()).select_one(
							_parents, _offsprings, _fitness_aux,
			...
			aux[i] =
					_setup.pool().selector(_setup.select_offsprings()).select_one(
							_offsprings, _offsprings, _fitness_aux,
							_setup.parameter_select_offsprings(), false);
...
Inter_Operator::Inter_Operator(const unsigned int _number_op,
		const Direction dir) :
		_number_operator(_number_op), direction(dir), migration_rate(1), migration_size(
				1), migration_selection_1(0), migration_selection_2(0), migration_selection_conf_1(
				0), migration_selection_conf_2(0) {
}
...
void Inter_Operator::RefreshState(const StateCenter& _sc) const {
	_sc.set_contents_state_variable("_migration_rate", (char *) &migration_rate,
			1, sizeof(migration_rate));
	_sc.set_contents_state_variable("_migration_size", (char *) &migration_size,
			1, sizeof(migration_size));
...
void Inter_Operator::UpdateFromState(const StateCenter& _sc) {
	...
	_sc.get_contents_state_variable("_migration_rate", (char *) &migration_rate,
			nitems, length);
	_sc.get_contents_state_variable("_migration_size", (char *) &migration_size,
			nitems, length);
...
void Migration::execute(Population& pop, const unsigned long current_generation,
		...
		if (synchronized)  // synchronous mode: blocked until data are received
		{
			...
			for (int i = 0; i < migration_size; i++) {
				...
				if ((solution_received->fitness()
						<= solution_to_remplace->fitness()
						&& direction == minimize)
						|| (solution_received->fitness()
								>= solution_to_remplace->fitness()
								&& direction == maximize)) {
...
void Selection_Roulette_Wheel::prepare(
	...
	if ((direction == maximize && (remplace))
			|| ((direction == minimize) && (!(remplace)))) {
		...
		for (int i = 0; i < fitness_values.size(); i++) {
			if (fitness_values[i].fitness != 0)
				fitness_values[i].sel_parameter =
						(1 / fitness_values[i].fitness);
			else
	...
	if (overall_fitness > DBL_MAX)
		overall_fitness = DBL_MAX;
...
Solver::Solver(const Problem& pbm, const SetUpParams& setup) :
		problem(pbm), params(setup), _stat(), _userstat(), _sc(), current_population(
				pbm, setup), best_cost((-1) * pbm.direction() * infinity()), worst_cost(
				(-1) * best_cost), best_solution(problem), average_cost(0.0), standard_deviation(
				0.0), time_spent_in_trial(0.0), total_time_spent(0.0), start_trial(
				0.0), start_global(0.0), _current_trial("_current_trial", _sc), _current_iteration(
				"_current_iteration", _sc), _current_evaluations(
				"_current_evaluations", _sc), _current_best_solution(
				"_current_best_solution", _sc), _current_best_cost(
				"_current_best_cost", _sc), _current_worst_cost(
				...
				"_best_sol_trial", _sc), _best_cost_trial("_best_cost_trial",
				_sc), _worst_cost_trial("_worst_cost_trial", _sc), _iteration_best_found_in_trial(
				"_iteration_best_found_in_trial", _sc), _evaluations_best_found_in_trial(
				"_evaluations_best_found_in_trial", _sc), _time_best_found_trial(
				"_time_best_found_trial", _sc), _time_spent_trial(
				"_time_spent_trial", _sc), _trial_best_found(
				...
				"_global_worst_cost", _sc), _time_best_found("_time_best_found",
				_sc), _crossover_probability("_crossover_probability", _sc), _mutation_probability(
				"_mutation_probability", _sc), _migration_rate(
				"_migration_rate", _sc), _migration_size("_migration_size",
				_sc), _migration_selection_1("_migration_selection_1", _sc), _migration_selection_2(
				"_migration_selection_2", _sc), _migration_selection_conf_1(
				"_migration_selection_conf_1", _sc), _migration_selection_conf_2(
				"_migration_selection_conf_2", _sc), _select_parents(
				"_select_parents", _sc), _select_offsprings(
				"_select_offsprings", _sc), _parameter_select_parents(
				"_parameter_select_parents", _sc), _parameter_select_offsprings(
				"_parameter_select_offsprings", _sc), _display_state(
				"_display_state", _sc) {
	current_trial(0);
	current_iteration(0);
...
unsigned int Solver::iteration_best_found_in_trial() const {
	unsigned int value = 0;
	unsigned long nitems, length;
	_iteration_best_found_in_trial.get_contents((char *) &value, nitems,
			length);
	return value;
}
...
unsigned int Solver::select_parents() const {
	...
	unsigned long nitems, length;
	_sc.get_contents_state_variable("_select_parents", (char *) &select_parents,
			nitems, length);
...
unsigned int Solver::parameter_select_parents() const {
	...
	unsigned long nitems, length;
	_sc.get_contents_state_variable("_parameter_select_parents",
			(char *) &parameter_select_parents, nitems, length);
...
unsigned int Solver::parameter_select_offsprings() const {
	...
	unsigned long nitems, length;
	_sc.get_contents_state_variable("_parameter_select_offsprings",
			(char *) &parameter_select_offsprings, nitems, length);
...
void Solver::current_standard_deviation(const double value) {
	_current_standard_deviation.set_contents((char *) &value, 1,
			sizeof(double));
}
...
void Solver::iteration_best_found_in_trial(const unsigned int value) {
	_iteration_best_found_in_trial.set_contents((char *) &value, 1,
			sizeof(int));
}
...
void Solver::user_op_probability(const int index,
		const float *new_probability) {
	char aux[30] = "_user_op_probability";
	sprintf(aux, "%s%d", aux, index);
...
void Solver::KeepHistory(const Solution& best_sol, const double best_cost,
	...
	case minimize:
		betterG = (best_cost < global_best_cost()
				|| (best_cost == global_best_cost()
						&& time_spent_in_trial < time_best_found()));
		worseG = (worst_cost > global_worst_cost());
		betterT = (best_cost < best_cost_trial()
				|| (best_cost == best_cost_trial()
						&& time_spent_in_trial < time_best_found_trial()));
...
void Solver_Lan::send_local_state_to(int _mypid) {
	...
	_netstream << pack_begin << _mypid << current_trial() << current_iteration()
			<< current_evaluations() << best_cost_trial()
			<< best_solution_trial() << iteration_best_found_in_trial()
...
void Solver_Lan::check_for_refresh_global_state() // Executed in process with pid 0
{
	...
	while (!_end_trial) {
		...
		if (received_pid == -1) {
			...
			if (!final_phase && terminateQ(problem, *this, params)) {
				...
				acum_evaluations = acum_iterations
						* params.population_additional_size()
	...
	case minimize:
		betterG = (best_cost < global_best_cost()
				|| (best_cost == global_best_cost()
						&& time_best_found_trial() <= time_best_found()));
		break;
	case maximize:
		betterG = (best_cost > global_best_cost()
				|| (best_cost == global_best_cost()
