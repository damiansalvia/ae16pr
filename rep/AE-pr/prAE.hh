...
// TODO - Declarar clases extra si se usan
...
requires bool terminateQ(const Problem& pbm, const Solver& solver,
...
private:
	int newValue(const int oldValue, const int l1, const int l2,
...
private:
	...
	void create_map(int **m, int *c_m, int **p, int *c_p, const Solution &s1,
...
private:
	Solution *crossAux(Solution &sol1, Solution &sol2) const;
	void create_map(int **m, int *c_m, int **p, int *c_p, Solution &s1,
			Solution s2) const;
	void remove_of_map(int c, int **m, int *c_m, int **p, int *c_p) const;
	int choose_random(int *c, int size) const;
...
public:
	StopCondition();
	virtual bool EvaluateCondition(const Problem& pbm, const Solver& solver,
			const SetUpParams& setup)=0;
	~StopCondition();
};
...
public:
	...
	void interchange(const unsigned long current_generation,
			NetStream& channel);
...
public:
	...
	virtual void execute(Population& pop,
...
public:
	...
	friend ostream& operator<<(ostream& os,
			const Selection_Roulette_Wheel& sel);
...
public:
	...
	virtual void run(const Population& pop,
			const unsigned long int nb_generations) =0;
	...
	void KeepHistory(const Solution& best_sol, const double best_cost,
			const double worst_cost, const float time_spent_trial,
...
public:
	Solver_Wan(const Problem& pbm, const SetUpParams& setup, int argc,
			char **argv);
	virtual ~Solver_Wan();
	virtual int pid() const;
	NetStream& netstream();
