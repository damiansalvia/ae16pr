...
if len(sys.argv) < 4:
	 raise Exception('estadisitcas.py <cantidad_de_ejecuciones> <path_matriz> <path_temporadas> [$HOME]')

# Get parametros
cantidad = int(sys.argv[1])
path_matriz = str(sys.argv[2])
path_temporadas = str(sys.argv[3])
if len(sys.argv) == 5: home = sys.argv[4]
...
# Commando
comando = 'ejercicio2.sh '+path_matriz+' '+path_temporadas+' '+home
#comando = "make SEQ "
...
soluciones = []
nombre = re.search('(?:.*\/)*(.*)_',path_matriz).group(1)
archSol = open('eval_%s.csv'%nombre, 'w')


for idx in range(cantidad):
	
	os.system(ruta+comando + ' > temp')
